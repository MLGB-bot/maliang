...
class MFont:
    ...
    def generate_pillow_temp_font(self, text='', text_size=12):
        ...
        font_runtime = pr.load_font_from_memory(self._type, self._bin, self._len, text_size, codepoints, codepoints_count[0])
    ...
    def text(self, text, x, y, text_size=None, text_color=None, space=0):
        font_runtime = self.generate_pillow_temp_font(text, text_size)
        pr.draw_text_ex(font_runtime, text, pr.Vector2(x, y), text_size, space, mod_color.MColor(*text_color).to_pyray())
